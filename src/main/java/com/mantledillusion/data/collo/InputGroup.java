...
/**
 * Defines a group of {@link InputPart} implementing {@link Enum} entries that
 * form a syntactic group.
 * <p>
 * The group's parts have to be separated by a specifiable separator, so they
 * can be matched individually.
 * <p>
 * To form an {@link InputGroup}, use {@link #forPart(Enum)} or
 * {@link #forPart(Enum, boolean)} to start an {@link InputGroupBuilder}.
 * 
 * @param <P>
 *            The {@link InputPart} implementing {@link Enum} type that defines
 *            the group's parts.
 */
public final class InputGroup<P extends Enum<P> & InputPart> {
	 ...
	 * <p>
	 * All {@link InputPart}s given to the builder will be contained and checked by
	 * the finally build group in the exact order they where given to the builder.
	 * 
	 * @param <P>
	 *            The {@link InputPart} implementing {@link Enum} type that defines
	 *            the group's parts.
	...
	public static final class InputGroupBuilder<P extends Enum<P> & InputPart> {
		 ...
		 * 
		 * @param inputPart
		 *            The part to add; might <b>not</b> be null.
		 * @return this
		 */
		 ...
		 *            The part to add; might <b>not</b> be null.
		 * @param isOptional
		 *            Whether the given part is optional in the group; or put
		 *            differently, whether it has to occur in an input term.
		 * @return this
		 */
		...
		/**
		 * Builds an {@link InputGroup} out of the {@link InputPart}s currently
		 * contained by this {@link InputGroupBuilder}.
		 * <p>
		 * <code>matchesAny</code> will be set to <code>true</code>; Even if all the
		 * group's parts are optional, at least one has to match a given term in order
		 * for the group to match.
		 * <p>
		 * <code>spliterator</code> will be set to <code>" "</code>; the group's parts
		 * are expected to be separated by a space.
		 * 
		 ...
		 * 
		 * @param matchesAny
		 *            Whether or not any of the group's parts has to match in a given
		 *            input term. This has only effect in groups whose parts are all
		 *            optional; in that case, using <code>false</code> will allow an
		 *            empty {@link String} to match the group.
		 * @return A new {@link InputGroup}; never null
		 */
		 ...
		 * 
		 * @param spliterator
		 *            The separator to split an input term by to gain separated
		 *            {@link InputPart}s; might <b>not</b> be null.
		 * @return A new {@link InputGroup}; never null
		 */
		...
		public InputGroup<P> build(boolean matchesAny, String spliterator) {
			...
			return new InputGroup<>(
					(InputPartOccurrence<P>[]) this.inputParts.toArray(new InputPartOccurrence[this.inputParts.size()]),
					matchesAny, spliterator);
		}
	}
	 ...
	 * @return True if the given term matches this {@link InputGroup}, false
	 *         otherwise
	 */
	...
	/**
	 * Returns a {@link List} of possibilities on how the given term can be split by
	 * this {@link InputGroup}'s separator to match this {@link InputGroup}'s
	 * {@link InputPart}s.
	 * <p>
	 ...
	 * 
	 * @param term
	 *            The term to analyze; might be null
	 * @return A {@link List} of possibilities how this group's parts match the
	 *         given term; never null, might be empty if this group does not match
	 *         the term at all (in this case, {@link #matches(String)} would return
